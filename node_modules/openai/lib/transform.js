...
function hasObjectConstraints(schema) {
...
function rewriteLocalRefsIntoFilteredAnyOfBranches(root) {
    const rewriteRef = (ref) => {
        ...
        for (const [index, part] of originalParts.entries()) {
            // SAFETY: This is an object schema at this traversal point; the record view permits dynamic keyword access while individual values remain subject to schema checks.
            const resolvedRecord = isObject(resolved) ? resolved : undefined;
            if (part === 'anyOf' &&
...
function mergeObjectAllOf(jsonSchema, path, root, normalizing = new Set()) {
    ...
    const objectBranches = resolvedEntries
        ...
        .filter((entry) => entry !== undefined && isMergeableObjectSchema(entry));
